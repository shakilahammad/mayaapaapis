...
class BkashController extends BkashPayment
{
    const CURRENCY = 'BDT';
    const INTENT = 'sale';
    ...
    public function checkoutNew()
    {
        ...
        return view('payment.bkash.checkout')->with([
            'amount' => $this->getDiscountedPriceNew($package, $params['userId']),
            'intent' => self::INTENT
        ]);
    }
...
//        "Maya Bot" => 0,
    ...
    public function createPayment($userId, $packageId)
    {
        try {
            ...
            $amount = $this->getDiscountedPriceNew($package, $user->id);
