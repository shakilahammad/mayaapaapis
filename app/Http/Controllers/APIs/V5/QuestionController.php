...
class QuestionController extends Controller
{
    ...
    {
        $question = Question::with(['tags'])->find($question_id);
...
//        $question['is_premium'] = $user->is_premium == 1 ? 1 : 0;
    ...
    private function fetchRequiredDetails($question)
    {
        ...
        if (count((array)$question->Answer)) {
...
//                }
        ...
        $values = [
            ...
            'spam_reason_en' => ($question->status == 'spam') ? $this->spamChecker($question->id, 'en') : '',
            'spam_reason_bn' => ($question->status == 'spam') ? $this->spamChecker($question->id, 'bn') : '',
            ...
            'specialist_profile_picture' => 'https://images-maya.s3.ap-southeast-1.amazonaws.com/images/userprofile/faruque_fahmid.jpeg' ?? '',
//            'specialist_profile_picture' => $expertProfile->profilePicture->url ?? '',
    ...
    private function storeQuestionView(Request $request, $question_id): void
    {
        try {
            ...
            if (count((array)$viewCount) < 1) {
                Question_view::create([
                    'user_id' => $request->user_id,
                    'question_id' => $question_id
                ]);
            }
            elseif((count((array)$viewCount) == 1)){
    ...
    private function spamChecker($q_id, $lang) {
        $spam_reason = json_decode(Miscellaneous::callAIApi($q_id));
//        dd($spam_reason);
        if(isset($spam_reason)&&!is_null($spam_reason->reason))
            return config('spam.'.$spam_reason->reason.'.'.$lang);
        else
            return config('spam.unknown.'.$lang);
    }
}
