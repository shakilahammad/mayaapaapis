...
class PremiumPackageController extends Controller
{
    ...
    public function getPackages_v3($userId = null, $nextPurchase = 0)
    {
        ...
        if ($isTrialFinished) {
            $packages = PremiumPackage::where('price', '>', 0)->where('status', '<>', 'inactive')->where('type', '<>', 'chat')->get();
        } else {
            $packages = PremiumPackage::where('status', '<>', 'inactive')->where('type', '<>', 'chat')->get();
        }
    ...
    public function getPackages_multisource($userId = null, $nextPurchase = 0)
    {
    ...
    public function getPremiumStatus($userId)
    {
        ...
        $payment = PremiumPayment::with(['premiumPackage'])
                    ->whereUserId($userId)
                    ->whereRaw('expiry_time > CURRENT_TIMESTAMP')
...
//                $this->fiftyPercentDiscountCode($userId),
    ...
    private function transformPackages_multisource($packages, $userId)
    {
        return $packages->map(function ($package) use ($userId) {
            return [
                'id' => $package->id,
                'name_en' => $package->name_en,
                'name_bn' => $package->name_bn,
    ...
    public function unsubscribe($user_id, $package_id){
        $package = PremiumPayment::where('user_id', $user_id)->where('package_id', $package_id)->where('status', 'active')->whereRAW('expiry_time > CURRENT_TIMESTAMP')->first();
        if(count($package)){
